...
weekly_data = df.groupby(pd.Grouper(key='datetime', freq='W-MON'))['nb_passages'].sum().reset_index()

# Split data into training (2019-2023) and testing (2024)
train_data = weekly_data[weekly_data['datetime'].dt.year < 2024]
test_data = weekly_data[weekly_data['datetime'].dt.year == 2024]
...
plt.title('Weekly Bike Trips')
plt.xlabel('Date')
plt.ylabel('Number of Trips')
plt.legend()
plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(millions_formatter))
plt.savefig('plots/weekly_bike_trips.png')
plt.close()

# Prepare the training data
y = train_data.set_index('datetime')['nb_passages']

# Use auto_arima to find the best parameters
model = auto_arima(y, start_p=1, start_q=1, max_p=3, max_q=3, m=52,
...
plt.title('Weekly Bike Trips - Forecast vs Actual')
plt.xlabel('Date')
plt.ylabel('Number of Trips')
plt.legend()
plt.gca().yaxis.set_major_formatter(ticker.FuncFormatter(millions_formatter))
plt.savefig('plots/forecast_vs_actual_weekly.png')
plt.close()
...
plt.xlabel('Date')
plt.ylabel('Residual')
plt.savefig('plots/residuals_weekly.png')
plt.close()
...
plt.savefig('plots/residuals_distribution_weekly.png')
plt.close()